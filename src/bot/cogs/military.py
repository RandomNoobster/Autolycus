...
class TargetFinding(commands.Cog):
    ...
    async def raids(self, ctx: discord.ApplicationContext):
        try:
            ...
            if not last_fetched or not file_content:
                ...
                return
            new_turn: bool = datetime.fromtimestamp(
                last_fetched).hour % 2 != 0 and datetime.utcnow().hour % 2 == 0

            embed1 = discord.Embed(
                title=f"Configuration", description="Do you want to use the same configuration (presenatation & filters) that you used last time running this command?", color=0xff5100)
            ...
            embed3 = discord.Embed(
                title=f"Filters (1/6)", description="What nations do you want to include?", color=0xff5100)
            embed4 = discord.Embed(
                title=f"Filters (2/6)", description="How many active defensive wars should they have?", color=0xff5100)
            embed5 = discord.Embed(
                title=f"Filters (3/6)", description="How inactive should they be?", color=0xff5100)
            embed6 = discord.Embed(
                title=f"Filters (4/6)", description="Do you want to include beige nations?", color=0xff5100)
            embed7 = discord.Embed(
                title=f"Filters (5/6)", description="Should there be a minimum previous beige loot?", color=0xff5100)
            ...
            option_list = [(embed1, stage_one()), (embed2, stage_two()), (embed3, stage_three()), (embed4, stage_four(
            )), (embed5, stage_five()), (embed6, stage_six()), (embed7, stage_seven()), (embed8, stage_eight())]
            ...
            done_jobs = [
                {"data": {"nations": {"data": file_content['nations']}}}]
            ...
            for done_job in done_jobs:
                for x in done_job['data']['nations']['data']:
                    ...
                    if x['wars'] != []:
                        ...
                        wars = sorted(
                            x['wars'], key=lambda k: k['date'], reverse=True)
                        war = wars[0]
                        if x['def_slots'] == 0:
                            x['time_since_war'] = (datetime.utcnow(
                            ) - datetime.strptime(war['date'], "%Y-%m-%dT%H:%M:%S%z").replace(tzinfo=None)).days
                        else:
                                ...
                                for attack in war['attacks']:
                                    ...
                                    if attack['loot_info']:
                                        ...
                                        if "won the war and looted" in text:
                                            nation_loot += utils.beige_loot_value(
                                                text, prices)
                                        else:
                                            continue
                                try:
            ...
            if not beige or who != "" or max_wars != 3 or performace_filter or inactive_limit != 0:
                ...
                if who != "":
                    if who == " alliance_position:[0,1]":
                        ...
                        filter_list.append(
                            "hide full alliance members and applicants")
                if max_wars != 3:
                    if max_wars == 0:
                ...
                if inactive_limit != 0:
                    filter_list.append(
                        f"hide nations that logged in within the last {inactive_limit} days")
                if minimum_beige_loot != 0:
                    filter_list.append(
                        f"hide nations with less than ${minimum_beige_loot:,} previous beige loot".replace(",000,000", "m"))
            ...
            for target in target_list:
                embed = discord.Embed(
                    title=f"{target['nation_name']}", url=f"https://politicsandwar.com/nation/id={target['id']}", description=f"{filters}\n\u200b", color=0xff5100)
                target['infrastructure'] = 0

                embed.add_field(name="Previous nation loot",
                                value=target["nation_loot"])
                ...
                if target['last_active'] == '-0001-11-30 00:00:00':
                    ...
                    days_inactive = (datetime.utcnow() - datetime.strptime(
                        target['last_active'], "%Y-%m-%dT%H:%M:%S%z").replace(tzinfo=None)).days

                for city in target['cities']:
                    target['infrastructure'] += city['infrastructure']

                embed.add_field(
                    name="Beige", value=f"{target['beige_turns']} turns")
                ...
                if target['alliance']:
                    embed.add_field(
                        name="Alliance", value=f"[{target['alliance']['name']}](https://politicsandwar.com/alliance/id={target['alliance_id']})\n{target['alliance_position'].lower().capitalize()}")
                else:
                ...
                embed.add_field(name="Chance to get ground IT",
                                value=str(round(100*ground_win_rate**3)) + "%")

                air_win_rate = self.winrate_calc(
                    (atck_ntn['aircraft'] * 3), (target['aircraft'] * 3))

                target['airwin'] = air_win_rate
                embed.add_field(name="Chance to get air IT",
                                value=str(round(100*air_win_rate**3)) + "%")

                naval_win_rate = self.winrate_calc(
                    (atck_ntn['ships'] * 4), (target['ships'] * 4))

                target['navalwin'] = naval_win_rate
                embed.add_field(name="Chance to get naval IT", value=str(
                    round(100*naval_win_rate**3)) + "%\n\u200b")
            ...
            if performace_filter:
                def determine(x):
                    if x['groundwin'] < .4 or x['nation_loot'] == "0" or x['net_cash_num'] < 10000:
                        ...
                        return True
                target_list[:] = [
                    target for target in target_list if determine(target)]
                if len(target_list) == 0:
                    await ctx.edit(content="No targets matched your criteria!", attachments=[])
                    no_timeout = True
                    return
            ...
            if webpage:
                ...
                webpage_embed = discord.Embed(title=f"Targets successfully gathered",
                ...
                def get_embed(nation):
                    ...
                    if "*" in nation['money_txt']:
                        embed.set_footer(
                            text=f"Page {cur_page}/{pages}  |  * the income if the nation is out of food.")
                    else:
                ...
                class embed_paginator(discord.ui.View):
                    ...
                    async def button_check(self, x):
                        beige_button = [
                            x for x in self.children if x.custom_id == "beige"][0]
                        user = await async_mongo.global_users.find_one({"user": ctx.author.id})
                ...
                targets = sorted(
                    best_targets, key=lambda k: k['nation_loot_value'], reverse=True)
                desc = filters
                ...
                embed = discord.Embed(
                    title="Top nations by beige loot", description=desc, color=0xff5100)
                embed.set_footer(
                    text="Contact RandomNoobster#0093 for help or bug reports")
    ...
    async def reminders(self, ctx: discord.ApplicationContext):
        try:
            ...
            if person['beige_alerts'] == []:
                insults = ['ha loser', 'what a nub', 'such a pleb',
                           'get gud', 'u suc lol', 'ur useless lmao']
            ...
            for n in range(0, len(reminders), 20):
                embed = discord.Embed(title="Beige reminders", description="".join(
                    reminders[n:n+30]), color=0xff5100)
                embeds.append(embed)
    ...
    async def delreminder(
        ...
        nation: Option(
            str, "Nation name, nation link, discord username etc of the nation whose beige reminder you want to remove")
    ):
        try:
            await ctx.defer()
            person = await async_mongo.global_users.find_one({"user": ctx.author.id})
    ...
    async def addreminder(
        ...
        ctx: discord.ApplicationContext,
        nation: Option(
            str, "Nation name, nation link, discord username etc of the nation you want to add a beige reminder for")
    ):
        try:
            await ctx.defer()
    ...
    async def battlesim(
        ...
        ctx: discord.ApplicationContext,
        nation1: Option(
            str, "Nation name, leader name, nation id, nation link or discord username. Defaults to your nation.") = None,
        nation2: Option(
            str, "Nation name, leader name, nation id, nation link or discord username. Defaults to your nation.") = None
    ):
        try:
            ...
            if results['nation2']['aircraft'] + results['nation1']['aircraft'] != 0:
                ...
                embed.add_field(name="Airstrike",
                                value="Nobody has any forces!")
                embed1.add_field(name="Airstrike",
                                 value="Nobody has any forces!")
    ...
    async def counters(
        ...
        try:
            ...
            embed = discord.Embed(
                title="Counters", description=f"[Explore counters against {result['nation_name']} on Slotter](https://slotter.bsnk.dev/search?nation={result['id']}&alliances={','.join(alliance_ids)}&countersMode=true&threatsMode=false&vm=false&grey=true&beige=false)", color=0xff5100)
            embed.set_footer(
                text="Slotter was made by Bann and is not affiliated with Autolycus")
    ...
    async def targetsheet(
        self,
        ctx: discord.ApplicationContext,
        allied_alliance_ids: Option(
            str, "The alliance id(s) to use when finding allied nations.") = [],
        enemy_alliance_ids: Option(
            str, "The alliance id(s) to use when finding enemy nations.") = []
    ):
        try:
            await ctx.defer()
            allied_id_list, id_str = utils.str_to_id_list(
                allied_alliance_ids or "")
            if id_str == "":
                try:
            ...
            enemy_id_list, id_str = utils.str_to_id_list(
                enemy_alliance_ids or "")
            if id_str == "":
                try:
            ...
            for enemy in enemy_nations:
                ...
                for ally in allied_nations:
                    ...
                    if enemy['score'] >= minscore and enemy['score'] <= maxscore:
                        ...
                        chances.append({"id": ally['id'], "winchance": (
                            results["nation1_ground_win_rate"] + results["nation1_air_win_rate"]) / 2})
                chances = sorted(
                    chances, key=lambda x: x['winchance'], reverse=True)
    ...
    async def war_status(
        ...
        ctx: discord.ApplicationContext,
        nation: Option(
            str, "The person whose war status you'd like to see") = None
    ):
        await ctx.defer()
        if not nation:
        ...
        embed = discord.Embed(
            title=f"{nation['nation_name']} ({nation['id']}) & their wars", description=desc, color=0xff5100)
        embed1 = discord.Embed(
            title=f"{nation['nation_name']} ({nation['id']}) & their wars", description=desc, color=0xff5100)
        ...
        class status_view(discord.ui.View):
            ...
            async def general_callback(self, b: discord.Button, i: discord.Interaction):
                ...
                other_buttons = [
                    x for x in self.children if x.custom_id != "status_general"]
            ...
            async def military_callback(self, b: discord.Button, i: discord.Interaction):
                ...
                other_buttons = [
                    x for x in self.children if x.custom_id != "status_military"]
                for button in other_buttons:
                    button.disabled = False
                this_button.disabled = True
            ...
            async def damage_callback(self, b: discord.Button, i: discord.Interaction):
                this_button = [
                    x for x in self.children if x.custom_id == "status_damage"][0]
                other_buttons = [
                    x for x in self.children if x.custom_id != "status_damage"]
                for button in other_buttons:
                    button.disabled = False
                this_button.disabled = True
    ...
    async def nuketargets(
        self,
        ctx: discord.ApplicationContext,
        sort: Option(str, "The metric to sort the targets by", choices=[
                     "Nuke damage", "Missile damage"]) = "Nuke damage",
        include_beige: Option(
            bool, "Include beige nations", default=False) = False,
        include_slotted: Option(
            bool, "Include slotted nations", default=False) = False
    ):
        try:
            ...
            for nation in all_nations:
                try:
                    ...
                    if not include_slotted:
                        ...
                        if def_wars == 3:
                            continue
                    nation['max_infra'] = sorted(
                        nation['cities'], key=lambda x: x['infrastructure'], reverse=True)[0]['infrastructure']
                    avg_infra = 0
            ...
            nation_list = sorted(
                nation_list, key=lambda x: x[sort_key], reverse=True)
            ...
            for n in range(0, len(nation_list), 8):
                ...
                embed.set_footer(
                    text=f"Page {n/8+1:.0f}/{math.ceil(len(nation_list)/8)}")
                embeds.append(embed)

            if len(embeds) > 1:
                view = utils.switch(ctx=ctx, embeds=embeds,
                                    max_page=len(embeds))
            else:
                view = None
    ...
    async def targets(
        ...
        try:
            ...
            embed = discord.Embed(
                title="Targets", description=f"[Explore your targets on slotter](https://slotter.bsnk.dev/search?nation={nation['id']}&alliances={','.join(alliance_ids)}&countersMode=false&threatsMode=false&vm=false&grey=true&beige=false)", color=0xff5100)
            embed.set_footer(
                text="Slotter was made by Bann and is not affiliated with Autolycus")
            await ctx.respond(embed=embed)
        except Exception as e:
    ...
    async def battle_calc(self, nation1_id=None, nation2_id=None, nation1=None, nation2=None):
        try:
            ...
            if nation1 and nation1_id or nation2 and nation2_id:
                raise Exception(
                    "You can't specify nation1 or nation2 multiple times!")
            if nation1:
            ...
            for war in results['nation1']['wars']:
                if war['attid'] == nation2_id and war['turnsleft'] > 0 and war['defid'] == nation1_id:
                    ...
                    # blockade is opposite than the others
                    if war['navalblockade'] == nation1_id:
            ...
            for attacker, defender in [("nation1", "nation2"), ("nation2", "nation1")]:
                defender_tanks_value = (
                    results[defender]['tanks'] * 40 * results[f'{defender}_tanks']) ** (3/4)
                defender_soldiers_value = (
                    results[defender]['soldiers'] * 1.75 + results[defender]['population'] * 0.0025) ** (3/4)
                defender_army_value = (
                    defender_soldiers_value + defender_tanks_value) ** (3/4)

                attacker_tanks_value = (
                    results[attacker]['tanks'] * 40 * results[f'{attacker}_tanks']) ** (3/4)
                attacker_soldiers_value = (
                    results[attacker]['soldiers'] * 1.75) ** (3/4)
                attacker_army_value = (
                    attacker_soldiers_value + attacker_tanks_value) ** (3/4)
                ...
                attacker_aircraft_value = (
                    results[attacker]['aircraft'] * 3) ** (3/4)
                defender_aircraft_value = (
                    results[defender]['aircraft'] * 3) ** (3/4)
                ...
                attacker_ships_value = (
                    results[attacker]['ships'] * 4) ** (3/4)
                defender_ships_value = (
                    results[defender]['ships'] * 4) ** (3/4)
                ...
                attacker_casualties_soldiers_value = utils.weird_division((attacker_soldiers_value**(4/3) + defender_soldiers_value**(
                    4/3)), (attacker_soldiers_value + defender_soldiers_value) * attacker_soldiers_value)
                defender_casualties_soldiers_value = utils.weird_division((attacker_soldiers_value**(4/3) + defender_soldiers_value**(
                    4/3)), (attacker_soldiers_value + defender_soldiers_value) * defender_soldiers_value)
                attacker_casualties_tanks_value = utils.weird_division((attacker_tanks_value**(
                    4/3) + defender_tanks_value**(4/3)), (attacker_tanks_value + defender_tanks_value) * attacker_tanks_value)
                defender_casualties_tanks_value = utils.weird_division((attacker_tanks_value**(
                    4/3) + defender_tanks_value**(4/3)), (attacker_tanks_value + defender_tanks_value) * defender_tanks_value)
                attacker_casualties_aircraft_value = utils.weird_division((attacker_aircraft_value**(4/3) + defender_aircraft_value**(
                    4/3)), (attacker_aircraft_value + defender_aircraft_value) * attacker_aircraft_value)
                defender_casualties_aircraft_value = utils.weird_division((attacker_aircraft_value**(4/3) + defender_aircraft_value**(
                    4/3)), (attacker_aircraft_value + defender_aircraft_value) * defender_aircraft_value)
                attacker_casualties_ships_value = utils.weird_division((attacker_ships_value**(
                    4/3) + defender_ships_value**(4/3)), (attacker_ships_value + defender_ships_value) * attacker_ships_value)
                defender_casualties_ships_value = utils.weird_division((attacker_ships_value**(
                    4/3) + defender_ships_value**(4/3)), (attacker_ships_value + defender_ships_value) * defender_ships_value)

                if results['gc'] == results[attacker]:
                    results[f'{attacker}_ground_{defender}_avg_aircraft'] = avg_air = round(min(
                        results[attacker]['tanks'] * 0.005 * results[f'{attacker}_ground_win_rate'] ** 3, results[defender]['aircraft']))
                ...
                for type, cas_rate in [("avg", 0.7), ("diff", 0.3)]:
                    ...
                    results[f'{attacker}_ground_{attacker}_{type}_soldiers'] = min(round(
                        ((defender_casualties_soldiers_value * 0.0084) + (defender_casualties_tanks_value * 0.0092)) * cas_rate * 3), results[attacker]['soldiers'])
                    results[f'{attacker}_ground_{attacker}_{type}_tanks'] = min(round((((defender_casualties_soldiers_value * 0.0004060606) + (defender_casualties_tanks_value * 0.00066666666)) * results[f'{attacker}_ground_win_rate'] + (
                    ...
                    results[f'{attacker}_ground_{defender}_{type}_soldiers'] = min(round(
                        ((attacker_casualties_soldiers_value * 0.0084) + (attacker_casualties_tanks_value * 0.0092)) * cas_rate * 3), results[defender]['soldiers'])
                    results[f'{attacker}_ground_{defender}_{type}_tanks'] = min(round((((attacker_casualties_soldiers_value * 0.00043225806) + (attacker_casualties_tanks_value * 0.00070967741)) * results[f'{attacker}_ground_win_rate'] + (
                ...
                results[f'{attacker}_airvair_{attacker}_avg'] = min(round(
                    defender_casualties_aircraft_value * 0.7 * 0.01 * 3 * results[f'{attacker}_extra_cas']), results[attacker]['aircraft'])
                results[f'{attacker}_airvair_{attacker}_diff'] = min(round(
                    defender_casualties_aircraft_value * 0.3 * 0.01 * 3 * results[f'{attacker}_extra_cas']), results[attacker]['aircraft'])
                results[f'{attacker}_airvother_{attacker}_avg'] = min(round(
                    defender_casualties_aircraft_value * 0.7 * 0.015385 * 3 * results[f'{attacker}_extra_cas']), results[attacker]['aircraft'])
                results[f'{attacker}_airvother_{attacker}_diff'] = min(round(
                    defender_casualties_aircraft_value * 0.3 * 0.015385 * 3 * results[f'{attacker}_extra_cas']), results[attacker]['aircraft'])

                results[f'{attacker}_airvair_{defender}_avg'] = min(round(
                    attacker_casualties_aircraft_value * 0.7 * 0.018337 * 3), results[defender]['aircraft'])
                results[f'{attacker}_airvair_{defender}_diff'] = min(round(
                    attacker_casualties_aircraft_value * 0.3 * 0.018337 * 3), results[defender]['aircraft'])
                results[f'{attacker}_airvother_{defender}_avg'] = min(round(
                    attacker_casualties_aircraft_value * 0.7 * 0.009091 * 3), results[defender]['aircraft'])
                results[f'{attacker}_airvother_{defender}_diff'] = min(round(
                    attacker_casualties_aircraft_value * 0.3 * 0.009091 * 3), results[defender]['aircraft'])

                results[f'{attacker}_naval_{defender}_avg'] = min(round(
                    attacker_casualties_ships_value * 0.7 * 0.01375 * 3 * results[f'{attacker}_extra_cas']), results[defender]['aircraft'])
                results[f'{attacker}_naval_{defender}_diff'] = min(round(
                    attacker_casualties_ships_value * 0.3 * 0.01375 * 3 * results[f'{attacker}_extra_cas']), results[defender]['aircraft'])
                results[f'{attacker}_naval_{attacker}_avg'] = min(round(
                    defender_casualties_ships_value * 0.7 * 0.01375 * 3), results[attacker]['aircraft'])
                results[f'{attacker}_naval_{attacker}_diff'] = min(round(
                    defender_casualties_ships_value * 0.3 * 0.01375 * 3), results[attacker]['aircraft'])
            ...
            for attacker, defender in [("nation1", "nation2"), ("nation2", "nation1")]:
                results[f'{attacker}_ground_{defender}_lost_infra_avg'] = max(min(((results[attacker]['soldiers'] - results[defender]['soldiers'] * 0.5) * 0.000606061 + (results[attacker]['tanks'] - (results[defender]['tanks'] * 0.5)) * 0.01) *
                ...
                results[f'{attacker}_ground_{defender}_lost_infra_diff'] = results[
                    f'{attacker}_ground_{defender}_lost_infra_avg'] / 0.95 * 0.15
                results[f'{attacker}_ground_loot_avg'] = (results[attacker]['soldiers'] * 1.1 + results[attacker]['tanks'] * 25.15) * (results[f'{attacker}_ground_win_rate'] ** 3) * 3 * 0.95 * results[f'{attacker}_war_loot_mod'] * \
                    results[f'{attacker}_policy_loot_stolen'] * results[f'{defender}_policy_loot_lost'] * \
                    results[f'{attacker}_pirate_econ_loot'] * \
                ...
                results[f'{attacker}_air_{defender}_lost_infra_avg'] = max(min((results[attacker]['aircraft'] - results[defender]['aircraft'] * 0.5) * 0.35353535 * 0.95 * results[f'{attacker}_air_win_rate'],
                ...
                results[f'{attacker}_air_{defender}_soldiers_destroyed_avg'] = round(max(min(results[defender]['soldiers'], results[defender]['soldiers'] * 0.75 + 1000, (
                    results[attacker]['aircraft'] - results[defender]['aircraft'] * 0.5) * 35 * 0.95), 0)) * airstrike_casualties(results[f'{attacker}_air_win_rate'])
                ...
                results[f'{attacker}_air_{defender}_tanks_destroyed_avg'] = round(max(min(results[defender]['tanks'], results[defender]['tanks'] * 0.75 + 10, (
                    results[attacker]['aircraft'] - results[defender]['aircraft'] * 0.5) * 1.25 * 0.95), 0)) * airstrike_casualties(results[f'{attacker}_air_win_rate'])
                results[f'{attacker}_air_{defender}_tanks_destroyed_diff'] = results[
                    f'{attacker}_air_{defender}_tanks_destroyed_avg'] / 0.95 * 0.1
                results[f'{attacker}_air_{defender}_ships_destroyed_avg'] = round(max(min(results[defender]['ships'], results[defender]['ships'] * 0.75 + 4, (
                    results[attacker]['aircraft'] - results[defender]['aircraft'] * 0.5) * 0.0285 * 0.95), 0)) * airstrike_casualties(results[f'{attacker}_air_win_rate'])
                results[f'{attacker}_air_{defender}_ships_destroyed_diff'] = results[
                    f'{attacker}_air_{defender}_ships_destroyed_avg'] / 0.95 * 0.1

                results[f'{attacker}_naval_{defender}_lost_infra_avg'] = max(min((results[attacker]['ships'] - results[attacker]['ships'] * 0.5) * 2.625 * 0.95 * results[f'{attacker}_naval_win_rate'],
                ...
                results[f'{attacker}_nuke_{defender}_lost_infra_avg'] = max(min((1700 + max(2000, results[defender]['city']['infrastructure'] * 100 / results[defender]['city']['land'] * 13.5)) / 2, results[defender]['city']
                                                                            ['infrastructure'] * 0.8 + 150), 0) * results[f'{attacker}_war_infra_mod'] * results[f'{attacker}_policy_infra_dealt'] * results[f'{defender}_policy_infra_lost'] * results[f'{defender}_fallout_shelter_mod']
                results[f'{attacker}_missile_{defender}_lost_infra_avg'] = max(min((300 + max(350, results[defender]['city']['infrastructure'] * 100 / results[defender]['city']['land'] * 3)) / 2,
                                                                               results[defender]['city']['infrastructure'] * 0.3 + 100), 0) * results[f'{attacker}_war_infra_mod'] * results[f'{attacker}_policy_infra_dealt'] * results[f'{defender}_policy_infra_lost']
                ...
                for infra in [
                    ...
                    results[f'{infra}_avg_value'] = utils.infra_cost(
                        results[defender]['city']['infrastructure'] - results[f'{infra}_avg'], results[defender]['city']['infrastructure']) * modifier

                for attack in ['airvair', 'airvsoldiers', 'airvtanks', 'airvships']:
                    results[f"{attacker}_{attack}_{defender}_lost_infra_avg_value"] = results[
                        f"{attacker}_air_{defender}_lost_infra_avg_value"] * 1/3
                results[f"{attacker}_airvinfra_{defender}_lost_infra_avg_value"] = results[f"{attacker}_air_{defender}_lost_infra_avg_value"]

                results[f'{attacker}_ground_{attacker}_mun'] = results[attacker]['soldiers'] * \
                    0.0002 + results[attacker]['tanks'] * 0.01
                results[f'{attacker}_ground_{attacker}_gas'] = results[attacker]['tanks'] * 0.01
                # -salvage(results[f'{attacker}_ground_win_rate'], results[f'{attacker}_ground_{defender}_alum'])
                results[f'{attacker}_ground_{attacker}_alum'] = 0
                results[f'{attacker}_ground_{attacker}_steel'] = results[f'{attacker}_ground_{attacker}_avg_tanks'] * 0.5 - salvage(
                    results[f'{attacker}_ground_win_rate'], results[f'{attacker}_ground_{attacker}_avg_tanks'] * 0.5) - salvage(results[f'{attacker}_ground_win_rate'], results[f'{attacker}_ground_{defender}_avg_tanks'] * 0.5)
                ...
                base_mun = (results[defender]['soldiers'] * 0.0002 + results[defender]['population'] / 2000000 +
                            results[defender]['tanks'] * 0.01) * def_rss_consumption(results[f'{attacker}_ground_win_rate'])
                ...
                base_gas = results[defender]['tanks'] * 0.01 * \
                ...
                for attack in ['air', 'airvair', 'airvinfra', 'airvsoldiers', 'airvtanks', 'airvships']:
                    results[f'{attacker}_{attack}_{attacker}_gas'] = results[
                        f'{attacker}_{attack}_{attacker}_mun'] = results[attacker]['aircraft'] / 4
                    base_gas = results[defender]['aircraft'] / 4 * \
                        def_rss_consumption(
                ...
                results[f'{attacker}_airvair_{attacker}_alum'] = results[f'{attacker}_airvair_{attacker}_avg'] * 5 - salvage(
                    results[f'{attacker}_air_win_rate'], results[f'{attacker}_airvair_{attacker}_avg'] * 5) - salvage(results[f'{attacker}_air_win_rate'], results[f'{attacker}_airvair_{defender}_avg'] * 5)
                ...
                results[f'{attacker}_airvair_{defender}_money'] = results[f'{attacker}_airvair_{defender}_avg'] * \
                    4000 + \
                ...
                results[f'{attacker}_airvinfra_{attacker}_alum'] = results[f'{attacker}_airvother_{attacker}_avg'] * 5 - salvage(
                    results[f'{attacker}_air_win_rate'], results[f'{attacker}_airvother_{attacker}_avg'] * 5) - salvage(results[f'{attacker}_air_win_rate'], results[f'{attacker}_airvother_{defender}_avg'] * 5)
                ...
                results[f'{attacker}_airvinfra_{defender}_money'] = results[f'{attacker}_airvother_{defender}_avg'] * \
                    4000 + \
                ...
                results[f'{attacker}_airvsoldiers_{attacker}_alum'] = results[f'{attacker}_airvother_{attacker}_avg'] * 5 - salvage(
                    results[f'{attacker}_air_win_rate'], results[f'{attacker}_airvother_{attacker}_avg'] * 5) - salvage(results[f'{attacker}_air_win_rate'], results[f'{attacker}_airvother_{defender}_avg'] * 5)
                ...
                results[f'{attacker}_airvsoldiers_{defender}_money'] = results[f'{attacker}_airvother_{defender}_avg'] * 4000 + \
                    results[f'{attacker}_air_{defender}_lost_infra_avg_value'] * 1/3 + \
                ...
                results[f'{attacker}_airvtanks_{attacker}_alum'] = results[f'{attacker}_airvother_{attacker}_avg'] * 5 - salvage(
                    results[f'{attacker}_air_win_rate'], results[f'{attacker}_airvother_{attacker}_avg'] * 5) - salvage(results[f'{attacker}_air_win_rate'], results[f'{attacker}_airvother_{defender}_avg'] * 5)
                ...
                results[f'{attacker}_airvtanks_{defender}_money'] = results[f'{attacker}_airvother_{defender}_avg'] * 4000 + \
                ...
                results[f'{attacker}_airvships_{attacker}_alum'] = results[f'{attacker}_airvother_{attacker}_avg'] * 5 - salvage(
                    results[f'{attacker}_air_win_rate'], results[f'{attacker}_airvother_{attacker}_avg'] * 5) - salvage(results[f'{attacker}_air_win_rate'], results[f'{attacker}_airvother_{defender}_avg'] * 5)
                ...
                results[f'{attacker}_airvships_{defender}_money'] = results[f'{attacker}_airvother_{defender}_avg'] * 4000 + \
                    results[f'{attacker}_air_{defender}_lost_infra_avg_value'] * 1/3 + \
                ...
                results[f'{attacker}_naval_{attacker}_steel'] = results[f'{attacker}_naval_{attacker}_avg'] * 30 + salvage(
                    results[f'{attacker}_naval_win_rate'], results[f'{attacker}_naval_{attacker}_avg'] * 30) + salvage(results[f'{attacker}_naval_win_rate'], results[f'{attacker}_naval_{defender}_avg'] * 30)
                ...
                base_mun = results[defender]['ships'] * 2.5 * \
                    def_rss_consumption(results[f'{attacker}_naval_win_rate'])
                ...
                base_gas = results[defender]['ships'] * 1.5 * \
                    def_rss_consumption(results[f'{attacker}_naval_win_rate'])
                ...
                results[f'{attacker}_nuke_{defender}_total'] = results[f'{attacker}_nuke_{defender}_alum'] * 2971 + results[f'{attacker}_nuke_{defender}_steel'] * 3990 + \
                    ...
                    results[f'{attacker}_nuke_{defender}_mun'] * \
                    1960 + results[f'{attacker}_nuke_{defender}_money']
